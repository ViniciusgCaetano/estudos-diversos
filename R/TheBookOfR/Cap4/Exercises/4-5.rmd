...
```{r}
sex.fac <- factor(sex)
party.fac <- factor(party, levels =  c("Maori", "Greens", "Other", "National", "Labour"))
```
...
```{r}
new.party.fac <- factor(c("National","Maori","Maori","Labour","Greens","Labour"), levels=levels(party.fac))
party.fac <- c(party.fac, new.party.fac)


new.sex.fac <- factor(c("M","M","F","F","F","M"), levels=levels(sex.fac))
sex.fac <- c(sex.fac, new.sex.fac)
```
...
confidence.fac <- cut(confidence, breaks = breaks, right = T, include.lowest = T, labels = c("Low", "Moderate", "High"))
```
